...
import { brainstorm as legalBrainstorm, buildLegalBrainstormContextString } from './legal/legalReasoningPartner';
import { addCoreSections } from './core/corePromptSections';
...
function buildBisonPrompt(userInput, state, recurrence, mode, recentHistory, isDeveloper, embodiedContext, phaseContext = {}) {
  ...
  // ── CRITICAL: Core identity + protocol sections (see ./core/corePromptSections) ──
...
export async function processInteraction(userInput, recentHistory = [], options = {}) {
  ...
  // 5z-legal. Legal Reasoning Partner — protected reasoning. Deterministic, local.
  // Lawful paths mapped fully; unlawful paths only as consequence. Never a how-to.
  let legalBrainstormResult = null;
  if (state.legalReasoningRequested) {
    legalBrainstormResult = legalBrainstorm(userInput);
  }
  ...
  let faceStateSnapshot = null;
  try {
    const faceResult = await faceInteract('talk');
    faceStateSnapshot = faceResult?.state || null;
    faceContext = buildFaceContextString(faceStateSnapshot);
  } catch (e) {}

  // 5z-eco. Ecosystem Intelligence (Package 23) — aggregate-only observation,
  // TTL-cached, never dominates. Full report on explicit audit request.
  let ecosystemObservation = null;
  const ecosystemAuditRequested = detectEcosystemAuditRequest(userInput);
  try {
    ecosystemObservation = await observeEcosystem({
      faceState: faceStateSnapshot,
      currentBandwidth: cognitiveLoad?.currentBandwidth ?? 100,
    });
  } catch (e) { ecosystemObservation = getLastObservation(); }
  if (ecosystemAuditRequested) {
    return {
      text: formatEcosystemReport(ecosystemObservation),
      mode: RESPONSE_MODES.EXPLORE,
      isGardenCandidate: false,
      state, recurrence,
      ecosystemObservation,
      provenance: { source: 'ecosystem_intelligence', generatedAt: new Date().toISOString(), computeMode: getComputeMode(options), isDeveloper: !!options.isDeveloper },
    };
  }
  ...
  const phaseContext = {
    ...
    legalBrainstormContext: legalBrainstormResult
      ? buildLegalBrainstormContextString(legalBrainstormResult, { unlawfulPush: state.unlawfulPathRequested, howToPush: state.unlawfulHowToRequested, lawyerRequested: state.lawyerRequested })
      : null,
  };
  ...
  return {
    ...
    legalBrainstorm: legalBrainstormResult ? { question: legalBrainstormResult.question, options: legalBrainstormResult.options.map(o => ({ id: o.id, label: o.label, legality: o.legality, reversibility: o.reversibility })), unlawfulPush: !!state.unlawfulPathRequested } : null,
